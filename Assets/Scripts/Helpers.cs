...
public static class Helpers
{
    ...
    public static bool IsMega(Pokemon p)
        => HasForm(p, "mega") || HasForm(p, "megax") || HasForm(p, "megay")
           || NameHas(p, "Mega ");

    public static bool IsGmax(Pokemon p)
        => HasForm(p, "gmax") || HasForm(p, "gigantamax")
           || NameHas(p, "Gigantamax");

    public static bool IsHisui(Pokemon p)
        => HasForm(p, "hisui")
           || NameHas(p, "(Hisui") || NameHas(p, "Hisuian");
    ...
    public static string GetGenTitle(int gen) => gen switch
    {
        1 => "Kanto (Gen 1)",
        2 => "Johto (Gen 2)",
        3 => "Hoenn (Gen 3)",
