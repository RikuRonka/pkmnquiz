...
public static class DexOrder
{
    ...
    private static readonly Dictionary<string, Entry> _byKey = new();
    private static readonly List<string> _sectionsInOrder = new();
    ...
    public static void LoadForGeneration(int gen)
    {
        ...
        foreach (var raw in ta.text.Split('\n'))
        {
            ...
            if (line.StartsWith("## "))
            {
                ...
                if (string.IsNullOrEmpty(_firstSection))
                    _firstSection = currentSection;
    ...
    {
        if (TryGetEntry(p.name, out var e))
            return e.index;

        if (p.aliases != null)
            foreach (var a in p.aliases)
                if (TryGetEntry(a, out e))
                    return e.index;
    ...
    public static string GetSection(Pokemon p, int gen)
    {
        ...
        return gen switch
        {
            ...
            9 => /* if you add paradox tagging later */
            "Paldea (Gen 9)",
    ...
    public static int GetSectionOrder(string name, int gen)
    {
        if (gen == 6)
        {
            if (string.Equals(name, "Kalos (Gen 6)"))
                return 0;
            if (string.Equals(name, "Mega Evolutions"))
                return 1;
        }

        if (gen == 8)
        {
            if (string.Equals(name, "Galar (Gen 8)"))
                return 0;
            if (string.Equals(name, "Gigantamax"))
                return 1;
            if (string.Equals(name, "Hisui"))
                return 2;
        }

        if (gen == 9)
        {
            if (string.Equals(name, "Paldea (Gen 9)"))
                return 0;
            if (string.Equals(name, "Kitakami"))
                return 1;
            if (string.Equals(name, "Blueberry"))
                return 2;
        }
